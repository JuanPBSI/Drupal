<?php
    // Peticion Sitio1
    $server = 'http://back-end.com/site1/rest/list';
    $username = "jesus";
    $password = "chucho123.,";
    $ch=curl_init();
    ...
    $json_sitio1=curl_exec($ch);
    curl_close($ch);

    $obj_sitio1 = json_decode($json_sitio1);
    ...
    $contenido=$contenido.'<form action="/scripts/getSitio1.php" method="POST">\n';
    $contenido=$contenido.'<input type="submit" value="Actualizar Sitio 1" />\n';
    ...
    foreach ($obj_sitio1 as $key => $value)
    {
        $contenido=$contenido.'                 <li><a href="http://get-data.com/?Sitio=1&Nodo='.$key.'">'.$obj_sitio1->{$key}->{'title'}.'</a></li>\n';
    }
    $contenido=$contenido.'            </ol>\n';
    $contenido=$contenido.'            </td>\n';
    $contenido=$contenido.'        </tr>\n';
    ...
    $query = "UPDATE node__body SET body_value = E'$contenido' WHERE entity_id=4";
